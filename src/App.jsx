...
useEffect(()=>{try{
  ...
  async function saveMateria(){
    ...
    try{
      const{data,error}=await supabase.from('materie').insert({utente_email:utente.email,nome:newMatNome.trim(),emoji:newMatEmoji,cover_image:coverImg}).select().single()
      if(error)throw error
