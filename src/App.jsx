...
const CSS = `
  :root{
    --primary:#4F46E5;--primary-dk:#3730A3;--primary-lt:#818CF8;
    --accent:#06B6D4;--accent-lt:#67E8F9;
    --violet:#7C3AED;--violet-lt:#A78BFA;
    --white:#FFFFFF;--gray:#F4F4F4;--gray2:#E4E4E4;
    --ink:#1A1A2E;--muted:#6B7280;--dark-bg:#0F0F2D;
    --gradient:linear-gradient(135deg,#4F46E5 0%,#7C3AED 50%,#06B6D4 100%);
    --gradient-dk:linear-gradient(135deg,#3730A3 0%,#6D28D9 50%,#0891B2 100%);
  }
  *{box-sizing:border-box;margin:0;padding:0;}
  body{font-family:'DM Sans',sans-serif;background:var(--primary);min-height:100vh;color:var(--ink);overflow-x:hidden;}
  ...
  @keyframes aiDone{0%{opacity:0;transform:translateX(-50%) scale(.85)}20%{opacity:1;transform:translateX(-50%) scale(1)}80%{opacity:1}100%{opacity:0}}

  .logo-icon{background:var(--dark-bg);border-radius:26px;border:1.5px solid rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
  .brand-name{font-family:'Syne',sans-serif;font-weight:800;color:var(--white);letter-spacing:-.5px;line-height:1.1;}
  .brand-name em{color:var(--accent-lt);font-style:normal;}
  .login-card .brand-name{color:var(--ink);}
  .login-card .brand-name em{color:var(--primary);}
  .loading-bar-track{width:220px;height:4px;background:rgba(255,255,255,.15);border-radius:99px;overflow:hidden;}
  .loading-bar-fill{height:100%;background:var(--accent);border-radius:99px;animation:loadBar 2.5s ease-in-out forwards;}

  .login-card{background:var(--white);border-radius:28px;padding:36px 32px 32px;width:100%;max-width:400px;box-shadow:0 8px 40px rgba(79,70,229,.25);}
  ...
  .field input,.select-field{width:100%;padding:14px 16px;background:#F8F9FF;border:1.5px solid var(--gray2);border-radius:12px;font-family:'DM Sans',sans-serif;font-size:.95rem;color:var(--ink);outline:none;transition:border .2s;}
  .field input:focus,.select-field:focus{border-color:var(--primary);}
  .select-field{cursor:pointer;margin-bottom:12px;appearance:none;}
  .error-msg{color:#DC2626;font-size:.85rem;text-align:center;margin-top:10px;font-weight:500;}

  .btn-primary{width:100%;padding:15px;background:var(--gradient);color:var(--white);border:none;border-radius:16px;font-family:'Syne',sans-serif;font-weight:700;font-size:1rem;cursor:pointer;transition:opacity .2s,transform .1s;margin-top:8px;}
  ...
  .btn-sm.primary{background:var(--gradient);color:white;border:none;}
  .btn-sm.danger{color:#DC2626;}
  .btn-sm.danger:hover{border-color:#DC2626;background:rgba(220,38,38,.05);}
  ...
  .back-btn{background:rgba(255,255,255,.15);border:none;border-radius:12px;padding:8px 14px;color:white;font-family:'DM Sans',sans-serif;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;font-size:.9rem;transition:background .2s;}
  .back-btn:hover{background:rgba(255,255,255,.25);}

  .top-bar{background:var(--gradient);padding:16px 20px 12px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;}
  ...
  .materia-card{background:var(--white);border-radius:20px;padding:18px 16px;cursor:pointer;box-shadow:0 4px 24px rgba(79,70,229,.08);transition:transform .15s,box-shadow .15s;display:flex;flex-direction:column;gap:10px;user-select:none;position:relative;}
  .materia-card:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(79,70,229,.15);}
  ...
  .add-card{background:rgba(79,70,229,.06);border:2px dashed rgba(79,70,229,.2);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;min-height:120px;}
  .add-card span{font-size:1.6rem;color:var(--primary);}
  .add-card p{font-size:.82rem;color:var(--primary);font-weight:600;}
  ...
  .argomento-row{background:var(--white);border-radius:16px;padding:14px 16px;cursor:pointer;box-shadow:0 2px 12px rgba(79,70,229,.06);display:flex;align-items:center;gap:10px;transition:transform .15s;user-select:none;}
  ...
  .argomento-top{background:var(--gradient);}
  ...
  .arg-tab{background:rgba(255,255,255,.15);border:none;border-radius:12px 12px 0 0;padding:10px 16px;color:rgba(255,255,255,.75);font-weight:600;font-size:.85rem;cursor:pointer;white-space:nowrap;transition:all .2s;flex-shrink:0;font-family:'DM Sans',sans-serif;}
  ...
  .fonte-row{background:var(--white);border-radius:14px;display:flex;align-items:center;gap:12px;padding:10px 14px;box-shadow:0 2px 8px rgba(79,70,229,.05);min-height:60px;}
  ...
  .fonte-btn:hover{color:var(--primary);}
  .fonte-btn.del:hover{color:#DC2626;}

  .fab{position:fixed;bottom:24px;right:20px;z-index:150;width:54px;height:54px;border-radius:50%;background:var(--gradient);border:none;color:white;font-size:1.7rem;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 6px 24px rgba(79,70,229,.4);transition:transform .2s;}
  ...
  .fonte-type-btn:hover{border-color:var(--primary);background:#F5F3FF;}
  ...
  .tool-card{background:var(--white);border-radius:18px;padding:16px;cursor:pointer;box-shadow:0 4px 24px rgba(79,70,229,.07);transition:transform .15s,box-shadow .15s;display:flex;flex-direction:column;gap:6px;position:relative;}
  .tool-card:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(79,70,229,.13);}
  ...
  .tool-cfg-btn{position:absolute;top:10px;right:10px;background:var(--gray);border:none;border-radius:8px;width:26px;height:26px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--muted);transition:all .2s;font-size:.75rem;}
  .tool-cfg-btn:hover{background:var(--primary);color:white;}

  .storico-row{background:var(--white);border-radius:16px;padding:14px 16px;box-shadow:0 2px 12px rgba(79,70,229,.06);display:flex;align-items:center;gap:10px;user-select:none;transition:border .15s;border:2px solid transparent;}
  .storico-row.sel{border-color:var(--primary);}
  .storico-info{flex:1;min-width:0;}
  .storico-tipo{background:rgba(79,70,229,.1);color:var(--primary);font-size:.72rem;font-weight:700;padding:2px 8px;border-radius:99px;text-transform:uppercase;letter-spacing:.5px;display:inline-block;margin-bottom:4px;}
  ...
  .chat-wrap{display:flex;flex-direction:column;flex:1;gap:10px;}
  .chat-header{display:flex;align-items:center;justify-content:space-between;padding:4px 0;}
  .chat-prompt-btn{background:var(--gray);border:1.5px solid var(--gray2);border-radius:10px;padding:6px 12px;font-size:.78rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:5px;color:var(--ink);transition:border-color .2s;}
  .chat-prompt-btn:hover{border-color:var(--primary);}
  ...
  .chat-bubble.user{background:var(--gradient);color:white;align-self:flex-end;border-radius:18px 18px 4px 18px;}
  .chat-bubble.ai{background:var(--white);color:var(--ink);align-self:flex-start;box-shadow:0 2px 12px rgba(79,70,229,.07);border-radius:18px 18px 18px 4px;}
  ...
  .btn-send{background:var(--gradient);border:none;border-radius:14px;width:46px;height:46px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:white;transition:opacity .2s;flex-shrink:0;}
  .btn-send:hover{opacity:.85;}
  .btn-send:disabled{opacity:.5;cursor:not-allowed;}
  .chat-len-btn{background:var(--gray);border:1.5px solid var(--gray2);border-radius:10px;padding:0 10px;height:46px;font-size:.78rem;font-weight:700;cursor:pointer;color:var(--ink);flex-shrink:0;transition:border-color .2s;}
  .chat-len-btn:hover{border-color:var(--primary);}
  ...
  .fp-edit-btn{background:var(--gray);border:none;border-radius:10px;padding:7px 12px;font-size:.8rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:5px;color:var(--ink);}
  .fp-edit-btn:hover{background:#EEF2FF;}
  ...
  .ai-spinner{width:44px;height:44px;border:3px solid var(--gray2);border-top-color:var(--primary);border-radius:50%;animation:spin .8s linear infinite;}
  .ai-loading p{font-size:.9rem;font-weight:500;animation:pulse 1.5s infinite;}

  /* AI Done popup */
  .ai-done-popup{position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:var(--gradient);color:white;padding:14px 24px;border-radius:18px;font-size:.9rem;font-weight:700;z-index:600;pointer-events:none;white-space:nowrap;box-shadow:0 8px 32px rgba(79,70,229,.4);animation:aiDone 3s ease forwards;}

  /* Output cfg sheet */
  .cfg-sheet-title{font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;margin-bottom:6px;}
  .cfg-sheet-sub{font-size:.82rem;color:var(--muted);margin-bottom:18px;}
  .cfg-row{display:flex;align-items:center;gap:10px;margin-bottom:12px;}
  .cfg-label{font-size:.82rem;font-weight:600;width:100px;flex-shrink:0;}
  .cfg-btns{display:flex;gap:4px;}
  .cfg-btn{min-width:36px;height:32px;border-radius:9px;border:1.5px solid var(--gray2);background:none;font-size:.8rem;font-weight:700;cursor:pointer;transition:all .2s;padding:0 8px;}
  .cfg-btn.active{background:var(--gradient);color:white;border:none;}
  .cfg-btn:hover:not(.active){border-color:var(--primary);}

  /* Prompt mode */
  .prompt-mode-opts{display:flex;flex-direction:column;gap:8px;margin-bottom:14px;}
  .prompt-mode-opt{display:flex;align-items:flex-start;gap:10px;padding:12px 14px;border:1.5px solid var(--gray2);border-radius:14px;cursor:pointer;transition:all .2s;}
  .prompt-mode-opt.active{border-color:var(--primary);background:#F5F3FF;}
  .prompt-mode-opt-icon{font-size:1.3rem;flex-shrink:0;margin-top:1px;}
  .prompt-mode-opt-title{font-weight:700;font-size:.88rem;}
  .prompt-mode-opt-desc{font-size:.77rem;color:var(--muted);margin-top:2px;}
  .prompt-custom-input{width:100%;min-height:80px;padding:12px 14px;border:1.5px solid var(--gray2);border-radius:12px;font-family:'DM Sans',sans-serif;font-size:.88rem;outline:none;resize:vertical;transition:border .2s;}
  .prompt-custom-input:focus{border-color:var(--primary);}
  ...
  .quiz-card{background:var(--white);border-radius:20px;padding:24px;box-shadow:0 4px 24px rgba(79,70,229,.07);}
  ...
  .quiz-opt:hover:not(:disabled){border-color:var(--primary);background:#F5F3FF;}
  ...
  .quiz-opt.wrong .quiz-letter{background:#DC2626;color:white;}
  .quiz-exp{font-size:.84rem;color:var(--muted);margin-top:12px;padding:12px 14px;background:#F8F9FF;border-radius:12px;line-height:1.65;border-left:3px solid var(--accent);}
  .quiz-score{display:flex;flex-direction:column;align-items:center;gap:12px;padding:32px 16px;text-align:center;}
  .quiz-score-circle{width:96px;height:96px;border-radius:50%;border:5px solid var(--primary);display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-weight:800;font-size:1.6rem;color:var(--primary);}
  ...
  .quiz-reveal{background:#F8F9FF;border-radius:14px;padding:14px;font-size:.88rem;line-height:1.65;border-left:3px solid #22c55e;}
  ...
  .fc-front{background:var(--gradient);color:white;}
  .fc-back{background:linear-gradient(135deg,var(--dark-bg),#1E1B4B);color:white;transform:rotateY(180deg);}
  ...
  .fc-nav button:hover:not(:disabled){border-color:var(--primary);}
  ...
  .map-root{background:var(--gradient);color:white;border-radius:18px;padding:16px 24px;text-align:center;font-family:'Syne',sans-serif;font-weight:800;font-size:1.05rem;box-shadow:0 4px 20px rgba(79,70,229,.3);max-width:280px;width:100%;}
  ...
  .map-node{background:var(--white);border-radius:14px;padding:12px 14px;text-align:center;font-family:'Syne',sans-serif;font-weight:700;font-size:.85rem;box-shadow:0 3px 14px rgba(79,70,229,.08);cursor:pointer;transition:all .2s;width:100%;border:2px solid var(--gray2);}
  .map-node:hover{border-color:var(--primary);transform:translateY(-2px);}
  .map-node.expanded{border-color:var(--primary);background:#F5F3FF;}
  ...
  .riassunto-sec{background:var(--white);border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(79,70,229,.06);}
  ...
  .riassunto-sec-hdr.open{border-bottom-color:var(--primary);}
  ...
  .profilo-top{background:var(--gradient);text-align:center;}
  .avatar{width:72px;height:72px;border-radius:50%;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-size:2rem;margin:12px auto;border:3px solid rgba(255,255,255,.4);font-family:'Syne',sans-serif;font-weight:800;color:white;}
  ...
  .settings-section{background:var(--white);border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(79,70,229,.06);}
  ...
  .provider-card{background:var(--white);border-radius:18px;padding:16px;box-shadow:0 2px 12px rgba(79,70,229,.06);border-left:4px solid transparent;}
  .provider-card.active-p{border-left-color:var(--accent);}
  ...
  .badge-active{background:rgba(6,182,212,.12);color:#0891B2;font-size:.72rem;font-weight:700;padding:3px 10px;border-radius:99px;}
  .badge-inactive{background:var(--gray);color:var(--muted);font-size:.72rem;font-weight:700;padding:3px 10px;border-radius:99px;}
  .admin-form{background:var(--white);border-radius:18px;padding:20px;box-shadow:0 2px 12px rgba(79,70,229,.06);}
  .admin-form h3{font-family:'Syne',sans-serif;font-weight:800;font-size:1rem;margin-bottom:16px;}
  .banner-active{background:rgba(6,182,212,.1);border:1.5px solid var(--accent-lt);border-radius:14px;padding:12px 16px;font-size:.88rem;font-weight:500;}
  .banner-active strong{color:var(--accent);}
  ...
  .emoji-opt.sel{border-color:var(--primary);background:#F5F3FF;}

  .ripasso-body{flex:1;padding:20px;display:flex;flex-direction:column;gap:14px;overflow-y:auto;}
  .ripasso-card{background:var(--white);border-radius:18px;padding:18px;box-shadow:0 2px 12px rgba(79,70,229,.06);}
  ...
  .ripasso-opt.sel{border-color:var(--primary);background:#F5F3FF;}
  .ripasso-opt:hover{border-color:var(--primary);}
  .ripasso-item{background:var(--white);border-radius:14px;padding:12px 16px;box-shadow:0 2px 8px rgba(79,70,229,.05);display:flex;justify-content:space-between;align-items:center;gap:10px;}
  ...
  .notifica{position:fixed;top:16px;left:50%;transform:translateX(-50%);background:var(--gradient);color:white;padding:14px 20px;border-radius:18px;font-size:.88rem;font-weight:600;z-index:550;display:flex;align-items:center;gap:12px;box-shadow:0 8px 32px rgba(79,70,229,.4);cursor:pointer;max-width:90vw;animation:slideIn .4s ease;}

  @media(max-width:380px){.materia-grid,.tools-grid{grid-template-columns:1fr;}}
`

function injectStyles(){if(document.getElementById('fb-css'))return;const el=document.createElement('style');el.id='fb-css';el.textContent=CSS;document.head.appendChild(el)}
...
const SUBJ={
  ...
  '📖':{bg:'#FFF1F2',fg:'#E11D48'},
  '🧬':{bg:'#F0FDF4',fg:'#15803D'},
  '⚗️':{bg:'#FFFBEB',fg:'#D97706'},
  '🏛️':{bg:'#F8FAFC',fg:'#475569'},
  '🎵':{bg:'#FDF4FF',fg:'#9333EA'},
  '🖥️':{bg:'#EFF6FF',fg:'#2563EB'},
  '🌿':{bg:'#F0FDF4',fg:'#16A34A'},
  '📐':{bg:'#FFF7ED',fg:'#C2410C'},
  '🔭':{bg:'#EFF6FF',fg:'#1D4ED8'},
  '🧠':{bg:'#FDF4FF',fg:'#7C3AED'},
  '💡':{bg:'#FEFCE8',fg:'#CA8A04'},
  '⚙️':{bg:'#F1F5F9',fg:'#475569'},
  '🌊':{bg:'#EFF6FF',fg:'#0369A1'},
  '🦅':{bg:'#FFF7ED',fg:'#B45309'},
  '🏆':{bg:'#FEFCE8',fg:'#B45309'},
}
...
function showAIDone(msg='✅ AI ha finito!'){
  const e=document.querySelector('.ai-done-popup');if(e)e.remove()
  const el=document.createElement('div');el.className='ai-done-popup';el.textContent=msg
  document.body.appendChild(el);setTimeout(()=>el.remove(),3100)
}
  ...
  return(
    <div className="sheet-ov" onClick={onClose}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        ...
        <div className="cfg-sheet-sub">Impostazioni per {tool==='quiz'?'Quiz Multipla':tool==='quiz-aperta'?'Quiz Aperta':tool==='flashcards'?'Flash Cards':tool==='riassunto'?'Riassunto':tool==='mappa'?'Mappa Concettuale':'Punti Chiave'}</div>

        {/* Lunghezza / numero per tutti */}
        {(isQuiz||isFC)?(
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const handler=e=>{
      ...
      const sheets=[sheetMat,sheetArg,sheetFontePicker,sheetTextFonte,sheetUrlFonte,sheetRename,sheetOutputCfg,sheetPromptMode]
  ...
  },[screen,fullpage,dialog,sheetMat,sheetArg,sheetFontePicker,sheetTextFonte,sheetUrlFonte,sheetRename,sheetOutputCfg,sheetPromptMode])

  function closeAllSheets(){setSheetMat(false);setSheetArg(false);setSheetFontePicker(false);setSheetTextFonte(false);setSheetUrlFonte(false);setSheetRename(null);setSheetOutputCfg(null);setSheetPromptMode(false)}
  ...
  async function uploadFile(file){
    try{
      ...
      if(ue){toast('Errore storage: '+ue.message);return}
      ...
      const{data:row,error:ie}=await supabase.from('fonti').insert({
        utente_email:utente.email,materia_id:curMateriaId,argomento_id:curArgId,
        nome:file.name,url:publicUrl,tipo:'file'
      }).select().single()
      if(ie){toast('Errore DB: '+ie.message);return}
      if(row){setFonti(p=>[...p,row]);toast('✓ '+file.name)}
    }catch(e){toast('Errore: '+e.message)}
  }
  async function addTextFonte(){
    if(!textFonteVal.trim())return
    const nome='Testo — '+new Date().toLocaleDateString('it-IT')
    const{data:row,error}=await supabase.from('fonti').insert({utente_email:utente.email,materia_id:curMateriaId,argomento_id:curArgId,nome,url:'',testo:textFonteVal.trim(),tipo:'text'}).select().single()
  ...
  async function addUrlFonte(){
    ...
    const{data:row,error}=await supabase.from('fonti').insert({utente_email:utente.email,materia_id:curMateriaId,argomento_id:curArgId,nome,url,tipo:'url'}).select().single()
    if(error){toast('Errore: '+error.message);return}
  ...
  function prepareFonti(){
    ...
    // Include filenames as context for the AI
  ...
  async function runTool(key,cfg=outputCfg,isBackground=false){
    ...
    try{
      ...
      if(key==='flashcards'){const c=parseFC(result);setFcCards(c);setFcIdx(0);setFcFlipped(false);setFullpage({title:'Flash Cards',type:'fc',data:c})}
        ...
        const qs=result.split('---').map(b=>b.trim()).filter(Boolean).map(b=>({dom:b.match(/DOMANDA:\s*(.+?)(?:\n|$)/)?.[1]?.trim()||'',risp:b.match(/RISPOSTA:\s*([\s\S]+?)(?:---|$)/)?.[1]?.replace(/---$/,'').trim()||''})).filter(q=>q.dom)
        setQuizData(qs);setOpenAnswers({});setOpenRevealed({});setFullpage({title:'Quiz Aperta',type:'quiz-aperta',data:qs})
      }
  ...
  return(<>
    ...
    {screen==='home'&&<div className="screen anim">
      ...
      <div className="home-body">
        {selMaterie.size>0&&<div className="sel-bar"><span>{selMaterie.size} selezionate</span><button className="btn-sm danger" onClick={()=>setDialog({icon:'🗑️',title:'Elimina materie?',msg:`Eliminare ${selMaterie.size} materie con tutti i contenuti?`,confirmLabel:'Elimina',danger:true,onConfirm:()=>deleteMaterie(selMaterie)})}>Elimina</button></div>}
        <div className="section-title">Le tue materie</div>
        <div className="materia-grid">
          {materie.map(m=>(
            <div key={m.id} className="materia-card"
              onClick={()=>{if(selMaterie.size>0){const n=new Set(selMaterie);n.has(m.id)?n.delete(m.id):n.add(m.id);setSelMaterie(n);return}setCurMateriaId(m.id);loadArgomenti(m.id);setScreen('argomenti')}}
    ...
    {screen==='argomento'&&<div className="screen anim">
      ...
      <div className="arg-body">
        ...
        {argTab==='fonti'&&<>
          ...
          {argFonti.length===0&&<div className="empty"><span>📂</span><p>Nessuna fonte. Usa + per aggiungere.</p></div>}
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {argFonti.map(f=>{
              ...
              return(
                <div key={f.id} className="fonte-row" style={{border:isSel?'2px solid var(--primary)':'2px solid transparent'}}
                ...
                >
                  ...
                  <button className="fonte-btn del" onClick={()=>deleteFonte(f)}>
        ...
        {/* ── STRUMENTI ── */}
        {argTab==='strumenti'&&<>
          {activeProvider?.provider==='deepseek'&&argFonti.some(f=>f.tipo==='file')&&<div style={{background:'rgba(79,70,229,.08)',border:'1.5px solid rgba(79,70,229,.2)',borderRadius:12,padding:'10px 14px',fontSize:'.82rem',color:'var(--primary)'}}>⚠️ DeepSeek non supporta file. Solo testo e link funzioneranno.</div>}
          <div className="tools-grid">
            {[
              ...
              {key:'flashcards',icon:'🃏',name:'Flash Cards',desc:'Carte FRONTE/RETRO flip 3D'},
              {key:'mappa',icon:'🗺️',name:'Mappa concett.',desc:'Mappa visuale interattiva'},
              {key:'punti',icon:'🎯',name:'Punti chiave',desc:'Lista concetti fondamentali'},
            ].map(t=>(
              <div key={t.key} className="tool-card">
                ...
                <div onClick={()=>runTool(t.key,outputCfg)}>
        ...
        {argTab==='chat'&&<div className="chat-wrap">
          <div className="chat-header">
            <span style={{fontSize:'.8rem',fontWeight:600,color:'var(--muted)'}}>Chat con le tue fonti</span>
            <button className="chat-prompt-btn" onClick={()=>setSheetPromptMode(true)}>
              {promptMode.mode==='learning'?'🎓':promptMode.mode==='custom'?'✍️':'🤖'}
              {promptMode.mode==='learning'?'Guida apprendimento':promptMode.mode==='custom'?'Prompt custom':'Predefinita'}
            </button>
          </div>
          <div className="chat-messages">
            {chatMsgs.length===0&&<div className="empty"><span>💬</span><p>Chatta con l'AI sulle tue fonti</p></div>}
            {chatMsgs.map((m,i)=>(
    ...
    {screen==='impostazioni'&&<div className="screen anim" style={{background:'var(--gray)'}}>
      ...
      <div style={{padding:20,display:'flex',flexDirection:'column',gap:14}}>
        ...
        <button className="btn-primary" onClick={()=>{toast('Salvato ✓');setScreen('profilo')}}>Salva</button>
      </div>
    </div>}
          ...
          {riassuntoData.map((sec,i)=>(
            <div key={i} className="riassunto-sec">
              ...
              {expandedSecs.has(i)&&<div className="riassunto-sec-body">
                {sec.items.map((it,j)=>it.type==='h3'?<h3 key={j}>{it.text}</h3>:it.type==='li'?<ul key={j}><li>{it.text}</li></ul>:<p key={j}>{it.text}</p>)}
              </div>}
            </div>
          ))}
        </div>}
    ...
    {sheetFontePicker&&<div className="sheet-ov" onClick={()=>setSheetFontePicker(false)}><div className="sheet" onClick={e=>e.stopPropagation()}>
      ...
      <div className="fonte-picker">
        {[
          ...
          {icon:'📄',label:'PDF',accept:'.pdf',multiple:true},
          {icon:'📝',label:'Word',accept:'.doc,.docx',multiple:true},
          {icon:'📊',label:'PowerPoint',accept:'.ppt,.pptx',multiple:true},
          {icon:'📈',label:'Excel',accept:'.xls,.xlsx',multiple:true},
        ].map((t,i)=>(
        ...
        <label className="fonte-type-btn"><span>📁</span><p>Altro file</p><input type="file" accept="*/*" multiple style={{display:'none'}} onChange={e=>{Array.from(e.target.files).forEach(uploadFile);setSheetFontePicker(false)}}/></label>
      </div>
    </div></div>}
    ...
    {sheetOutputCfg&&<OutputCfgSheet
      ...
      onGenerate={()=>{const{tool,cfg}=sheetOutputCfg;setOutputCfg(cfg);setSheetOutputCfg(null);runTool(tool,cfg)}}
