// api/ai.js — FlashBacon v4 (ESM)
// AI risponde SOLO in base alle fonti caricate — nessuna ricerca web
...
// Fetch text content from a URL for inclusion in prompt
...
// ── BUILD SYSTEM PROMPT: sources-only ──
function buildSystemPrompt(extraText='', settings={}) {
  const lengthDesc = ['breve e concisa','di media lunghezza','lunga e dettagliata'][( settings.length||2)-1]
  const depthDesc  = ['semplice, senza tecnicismi','con approfondimento moderato','molto approfondita e tecnica'][(settings.depth||2)-1]
  return `Sei FlashBacon AI, un assistente di studio. Rispondi ESCLUSIVAMENTE in base al contenuto delle fonti fornite dall'utente (immagini, documenti, testi). NON usare conoscenze esterne. Se l'utente chiede esplicitamente di integrare con ricerche web, puoi farlo solo in quel caso. Rispondi sempre in italiano. La risposta deve essere ${lengthDesc} e ${depthDesc}.${extraText?' Fonti testuali aggiuntive:\n'+extraText:''}`
}

// ── GEMINI: URL-based (zero server download) ──
...
async function callAnthropic(apiKey, model, prompt, imageUrls, systemPrompt) {
  ...
  for (const url of imageUrls.slice(0,2)) {
    try {
      ...
      const tp = mt==='application/pdf' ? 'document' : 'image'
      content.push({ type:tp, source:{ type:'base64', media_type:mt, data:b64 } })
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch URL sources as text (fast, no heavy binary)
